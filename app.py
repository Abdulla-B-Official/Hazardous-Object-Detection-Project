...
torch.set_num_threads(1)  # Restrict PyTorch CPU threads to reduce RAM overhead
torch.set_grad_enabled(
    False
)  # Disable autograd calculations (inference only)

app = Flask(__name__)
CORS(app)

# Load YOLO model explicitly onto CPU
# Note: Ensure path points to your weights file (e.g., "best.pt" or "runs/hazardous_detection/weights/best.pt")
MODEL_PATH = (
    "best.pt"
    if os.path.exists("best.pt")
    else "runs/hazardous_detection/weights/best.pt"
)
...
def predict():
  ...
  try:
    ...
    result = model.predict(
        ...
        device="cpu",
        verbose=False,
        max_det=20,  # Limit max predictions per frame
    )[0]
    ...
    # Convert annotated image back to Base64 (using JPEG compression to reduce payload size)
